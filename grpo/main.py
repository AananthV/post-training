...
from model_utils_hf import load_model_and_tokenizer, get_peft_model
from inference_hf import run_fast_inference
...
def main():
    ...
    grpo_dataset, max_prompt_length = prepare_grpo_dataset(
        tokenizer, config.system_prompt
    )

    reward_manager = RewardManager(
        tokenizer, config.reasoning_end, config.solution_start, config.solution_end
    )
    ...
    response = run_fast_inference(
        model, tokenizer, messages, lora_request=model.load_lora("grpo_saved_lora")
    )
